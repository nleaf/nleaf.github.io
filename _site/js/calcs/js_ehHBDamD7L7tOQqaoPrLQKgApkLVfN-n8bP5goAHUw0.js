...
(function ($) {
  $(document).ready(function() {
			var updateto = setTimeout(function(){updatevalues()},1);
			function updatevalues() {
				$( "#nobcabinets" ).val($( "#nobcabinets-slider" ).slider( "value" ));
				$( "#kilowats" ).val($( "#kilowats-slider" ).slider( "value" ));
				$( "#evalperiod" ).val($( "#evalperiod-slider" ).slider( "value" ));
				$( "#powerpercentage" ).val($( "#powerpercentage-slider" ).slider( "value"));
				$( "#powercost" ).val($( "#powercost-slider" ).slider( "value"));		
				$( "#bandwidth" ).val($( "#bandwidth-slider" ).slider( "value"));
        $( ".tierhdr" ).html($("#dctier").val()+" COLOCATION SPACE");
				
				C4 = $("#dctier").val();
				C6 = parseFloat($( "#nobcabinets-slider" ).slider( "value" ));
        if (C6 < 10) {
          c6 = 10;
        }
				C7 = parseFloat($( "#kilowats-slider" ).slider( "value" ));
				E7 = C7
				C8 = C6*25;
				
				H4 = parseFloat($( "#evalperiod-slider" ).slider( "value" ));
				H5 = parseFloat($( "#powerpercentage-slider" ).slider( "value" ));
				H6 = parseFloat($( "#powercost-slider" ).slider( "value" ));
				H7 = parseFloat($( "#bandwidth-slider" ).slider( "value" ));
				H8 = $("#s247").val();
				K4 = H4;
				K5 = H5;
				K6 = H6;
				...
				$("#sqrft").val(C8);
				$("#wpsqft").val((C7*1000/C8).toFixed(0));
				...
				for (var i=12;i<34;i++) {
					...
					$("#cell-c"+i).html("$" + (E34*multiplierA).formatMoney(2, '.', ','));
					$("#cell-d"+i).html("$" + (E34*multiplierB).formatMoney(2, '.', ','));
					$("#cell-e"+i).html("$" + ((E34*multiplierA)+ (E34*multiplierB)).formatMoney(2, '.', ',') );
				}
        $(".subtotal").each(function() {
          total = 0;
          $("#"+$(this).attr("tbleref")).find("tr td:nth-child(4) span").each(function() {
            //console.log( $(this).html().replace(/[^\d\.\-\ ]/g, '') );
            total = total + parseFloat($(this).html().replace(/[^\d\.\-\ ]/g, ''));
          });
          $(this).html("$" +total.formatMoney(2, '.', ','));
        });
				$("#table1-total").val("$" + E34.formatMoney(0, '.', ','));
				...
				//(C7<20 ? 20:C7)
				function gcval(cell) {
					value = $("#cell-"+cell.toLowerCase()).html();
				...
				for (var i=12;i<20;i++) {
					...
					switch(i) {
						...
						case 14:
							...
							resultsB = E7*K6/100*720*K4*12*((K5<50?50:K5)/100)*4.2;
							break;
						case 15:
							resultsA = parseFloat(gcval("C16")+gcval("C17")+gcval("C18")+gcval("C19")+gcval("C20")+gcval("C24"))/10;
							resultsB = 0;
							break;
						case 16:
							resultsA = gcval("C25")*0.032*K4;
							resultsB = 0;
							break;
						case 17:
							resultsA = (gcval("C29")+gcval("C30")+gcval("C31")+gcval("C32"))*0.032*K4;
							resultsB = 0;
							break;
						case 18:
					...
					$("#cell-i"+i).html("$" + (resultsA).formatMoney(2, '.', ','));
					$("#cell-k"+i).html("$" + (resultsB).formatMoney(2, '.', ','));
				}
        $("#buildsummary").html("$" + (totalA).formatMoney(0, '.', ','));
				$("#table2-total1").val("$" + (totalA).formatMoney(0, '.', ','));
        
				$("#colocsummary").html("$" + (totalB).formatMoney(0, '.', ','));
				$("#table2-total2").val("$" + (totalB).formatMoney(0, '.', ','));
			}
			...
			if (!jQuery.browser.mobile) {
			  $(".alwaysontop-wrap").height( 
			    $(".alwaysontop-content").height() 
		    ).css("position","relative");
		    $(".alwaysontop-content").css({
		      'position' : 'absolute',
		      'top' : '0px;'		      
	      });
        ...
        $(window).scroll(function() {
          if (!jQuery.browser.mobile && $(window).width() > 1000 ) {
	          tbheight = 0;
	          if(!$("#toolbar").length == 0) {
      	      tbheight = $("#toolbar").height();
	          }
	          
            scrolltop = $(window).scrollTop();
            totop = $(".alwaysontop-wrap").offset().top;
            
            if (scrolltop > 1500) {
              tbheight = tbheight - (scrolltop - 1500);
            }
            
            if(scrolltop > (totop-tbheight)) {
              //$(".alwaysontop-content").css("top", (scrolltop-totop) +"px");
              $(".alwaysontop-content").addClass("ontop");
              $(".alwaysontop-content").css({
                "position" : "fixed",
                "top" : tbheight+"px"
              });
              $("form[name=calculator]").addClass("shrink"); 
            }
            else {
              $("form[name=calculator]").removeClass("shrink");
              $(".alwaysontop-content").css("top","0px");
              $(".alwaysontop-content").removeClass("ontop");
              $(".alwaysontop-content").css("position","absolute");
            }
          }
        });
			}
			*/
			...
			function updateTemperatureGauge() {
			  orgvalue = $("input[name=wpsqft]").val();
			  value = orgvalue;
			  // Get the relative percentage of the value
        ...
        if(orgvalue < 80) {
          if(!$("span.low").hasClass("on")) {
            $("#temperature-gauge .ranges span").removeClass("on")
            $("span.low").addClass("on");
          }
        }
        else if($("input[name=wpsqft]").val() < 151) {
          if(!$("span.average").hasClass("on")) {
            $("#temperature-gauge .ranges span").removeClass("on")
            $("span.average").addClass("on");
          }
        }    
        else if($("input[name=wpsqft]").val() < 200) {
          if(!$("span.medium").hasClass("on")) {
            $("#temperature-gauge .ranges span").removeClass("on")
            $("span.medium").addClass("on");
          }
        }   
        else {
          if(!$("span.high").hasClass("on")) {
            $("#temperature-gauge .ranges span").removeClass("on")
            $("span.high").addClass("on");
          }
        }                            
			  // Multiple that percentage by the physcial
			  // scale (in pixels) of the graph
			  value = value * ($("#temperature-gauge").width()-$("#temperature-gauge .thumb").width()/2);
			  $("#temperature-gauge .thumb").css({
			    'left' : value
			  });
			}
			...
			/*********************************************************************************************************/
      $(".calculator .field input").change(function() {
        updateto=setTimeout(function(){updatevalues();},1);
      });
			...
			$( "#nobcabinets-slider" ).slider({
				range: "min",			
				value:$("#nobcabinets").val(),
				min: 1,
				max: 500,
				step: 1,
				slide: function( event, ui ) {
					$("#nobcabinets").val( ui.value );
					clearTimeout(updateto);
					updateto=setTimeout(function(){updatevalues();},1);
				}
			});
			...
			$( "#evalperiod-slider" ).slider({
				range: "min",			
				value:$( "#evalperiod" ).val(),
				min: 1,
				max: 10,
				step: 1,
				slide: function( event, ui ) {
					$( "#evalperiod" ).val( ui.value );
					clearTimeout(updateto);
					updateto=setTimeout(function(){updatevalues();},1);
				}
			});
			...
			$( "#powerpercentage-slider" ).slider({
				range: "min",			
				value:$( "#powerpercentage" ).val(),
				min: 10,
				max: 100,
				step: 5,
				slide: function( event, ui ) {
					$( "#powerpercentage" ).val( ui.value);
					clearTimeout(updateto);
					updateto=setTimeout(function(){updatevalues();},1);
				}
			});
...
(function ($) {
  $(document).ready(function() {
    window.printpdf.print = function() {
      var title = "Print PDF | Build vs Buy";
      var data = "";
      data += $('<div/>').append($(".body > h1").clone().addClass("bvbmain")).html();
      data += $('<div/>').append($(".body p.description").clone()).html();
      data += "<h1>INPUT:</h1>";
      // *******************************
      //          Options
      // *******************************
      //data += "<h2>Selected Options</h2>";
      data += window.printpdf.drawTable([
        //Row 1
        [
          //Column 1
          [
            window.printpdf.printOption("#dctier")
          ],
          //Column 2
          [
            window.printpdf.printOption("#s247")
          ]  
        ]
      ], "#input-options");
      // *******************************
      ...
      data += window.printpdf.drawTable([
        ...
        [
          //Column 1
          [
            window.printpdf.printSlider("#nobcabinets")
          ],
          //Column 2
          [
            window.printpdf.printSlider("#powerpercentage")
          ]  
        ],
        //Row 2
        [
          //Column 1
          [
            window.printpdf.printSlider("#kilowats")
          ],
          //Column 2
          [
            window.printpdf.printSlider("#powercost")
          ]  
        ],
        //Row 3
        [
          //Column 1
          [
            window.printpdf.printSlider("#evalperiod")
          ],
          //Column 2
          [
            window.printpdf.printSlider("#bandwidth")
          ]  
        ]
      ], "#input-sliders");
      ...
      //          Temp Gague
      // *******************************
      data += "<div class='temp-results'>";
      data += window.printpdf.drawTable([
        //Row 1
        [
          //Column 1
          [
            "<h2>Power Density Meter</h2>"+
            "<p>"+$("#temperature p").html()+"</p>"
          ],
          //Column 2
          [
            window.printpdf.printTempGauge()
          ]  
        ]
      ],"#temp");
      ...
      rowiterator = 0;
      data += "<div class='results'>";
      data += "<h1>TOTAL COST **</h1>";
      data += "<table>";
      $("#table1 tr").each(function() {
        $(this).find("th").attr("width","");
        if (rowiterator == 0) {
          data += "<tr class='first'>";
        }        
        else if (window.printpdf.isEven(rowiterator)) {
          data += "<tr class='even'>";
        }
        else {
          data += "<tr class='odd'>";
      ...
      $("#table1A tr, #table1B tr, #table1C tr, #table1D tr").not(":first-child").each(function() {
        if (window.printpdf.isEven(rowiterator)) {
          data += "<tr class='even'>";
        }
        else {
          data += "<tr class='odd'>";
      ...
      tmptable = $(".results-table-total").not(".results-table-total2").clone();
      $(tmptable).find("input").remove();
      data += window.printpdf.drawTable([
        [
          //Column 1
          [
            tmptable.find("p").wrap("<div />").parent().html()
          ],
          //Column 2
          [
            "<h2 class='total'>"+$("#table1-total").val()+"</h2>"
          ]
        ]
      ],'#results01');
      
      rowiterator = 0;
      data += "<div style='height:10px;width:10px;'></div>";
      data += "<h1>DATA CENTER: TOTAL COST OF OWNERSHIP</h1>";
      temp = $("#totalownership").clone();
      $(temp).find("td, th").attr("width","");
      
      data += "<table id='totalownership'>";
      $(temp).find("tr").each(function() {
        $(this).find("td:nth-child(1),th:nth-child(1)").addClass("first");
        if (rowiterator == 0) {
          data += "<tr class='first'>";
        }        
        else if (window.printpdf.isEven(rowiterator)) {
      ...
      data += "<tr>";
        data += "<td></td>";
        data += "<td>";
          data += "<h2 class='total'>"+$("#table2-total1").val()+"</h2>";
        data += "</td>";
        data += "<td>";
          data += "<h2 class='total'>"+$("#table2-total2").val()+"</h2>"
        data += "</td>";      
      data += "</tr>";
      data += "</table>";
      data += "<div style='height:10px;width:10px;'></div>";
      tmp = $("#results-sections > p.disclaimer").clone();
      data += $(tmp).wrap("<div />").parent().html();
      data += "</div>";
