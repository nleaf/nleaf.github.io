...
(function ($) {
  $(document).ready(function() {
			...
			function updatevalues() {
				dbincrement = $("select#dbsizeincrement").val();
                                fileincrement = $("select#filesizeincrement").val();
                                calculatedincrement = 'MB';
				$( "#hours" ).val($( "#hours-slider" ).slider( "value" ) + " Hours");
				$( "#fs-data" ).val($( "#fs-data-slider" ).slider( "value" ) + " " + fileincrement);
				$( "#db-data" ).val($( "#db-data-slider" ).slider( "value" ) + " " + dbincrement);
				$( "#retention" ).val($( "#retention-slider" ).slider( "value" ) + " Weeks");
				
				
				E4 = parseFloat($( "#hours-slider" ).slider( "value" ));
				E5 = parseFloat($( "#fs-data-slider" ).slider( "value" ));
				E6 = parseFloat($( "#db-data-slider" ).slider( "value" ));
				E7 = parseFloat($( "#retention-slider" ).slider( "value" ));
                                
                                if(dbincrement == "TB") {
                                        E6 = E6 * 1000 * 1000;
                                        
                                }
                                else if (dbincrement == "GB") {
                                        E6 = E6 * 1000;
                                ...
                                else if (fileincrement == "GB") {
                                        E5 = E5 * 1000;
                                        
                                }     
                                totalstorage=parseInt((E5+E6)+(((0.014*E6)+(0.003*E5))*(7 *E7)));
                                                           
                                if (totalstorage > 999999) {
                                        calculatedincrement = "TB";
                                        totalstorage = totalstorage/1000/1000;
                                }
                                else if (totalstorage > 999) {
                                        calculatedincrement = "GB";
                                        totalstorage = totalstorage/1000;
                                }
                                
                                bandwithneeded=(totalstorage*8)/(E4*3600);
                                
                                bandwithunit = "Mbps";
                                switch(calculatedincrement) {
                                        case "TB":
                                                if (bandwithneeded < 1) {
                                                        bandwithunit = "Gbps";
                                                        bandwithneeded = bandwithneeded *1000;
                                                }
                                                else {
                                                        bandwithunit = "Tbps";
                                                }
				...
				$("#total-storage .result").html(roundNum(totalstorage) + " " + calculatedincrement );
						
				$("#bandwith-needed .result").html(roundNum(bandwithneeded) + " " + bandwithunit);
			}
			/*********************************************************************************************************/
			/*										       SLIDERS	   											 */
			/*********************************************************************************************************/
      $(".calculator .field input").change(function() {
        updateto=setTimeout(function(){updatevalues();},25);
			...
			$( "#fs-data-slider" ).slider({
				range: "min",			
				value:superClean($( "#fs-data" ).val()),
				min: 0,
				max: 1024,
				step: 1,
				slide: function( event, ui ) {
					$( "#fs-data" ).val( ui.value + " " + $("select#filesizeincrement").val());
					clearTimeout(updateto);
					updateto=setTimeout(function(){updatevalues();},25);
				}
			});
			$( "#fs-data" ).val($( "#fs-data-slider" ).slider( "value" ) + " " + $("select#filesizeincrement").val());

			$( "#db-data-slider" ).slider({
				range: "min",			
				value:superClean($( "#db-data" ).val()),
				min: 0,
				max: 1024,
				step: 1,
				slide: function( event, ui ) {
					$( "#db-data" ).val( ui.value + " " + $("select#dbsizeincrement").val());
					clearTimeout(updateto);
					updateto=setTimeout(function(){updatevalues();},25);
				}
			});
			$( "#db-data" ).val($( "#db-data-slider" ).slider( "value" ) + " " + $("select#dbsizeincrement").val());
			
			$( "#retention-slider" ).slider({
				range: "min",			
				value:superClean($( "#retention" ).val()),
				min: 1,
				max: 52,
				step: 1,
				slide: function( event, ui ) {
					$( "#retention" ).val( ui.value + " Weeks");
					clearTimeout(updateto);
					updateto=setTimeout(function(){updatevalues();},25);
				}
			});
    ...
    window.printpdf.print = function() {
      ...
      data += window.printpdf.drawTable([
        ...
        [
          //Column 1
          [
            window.printpdf.printOption("#dbsizeincrement")
          ],
          //Column 2
          [
            window.printpdf.printOption("#filesizeincrement")
          ]  
        ]
      ...
      data += window.printpdf.drawTable([
        ...
        [
          //Column 1
          [
            window.printpdf.printSlider("#hours")
          ],
          //Column 2
          [
            window.printpdf.printSlider("#fs-data")
          ]  
        ],
        //Row 2
        [
          //Column 1
          [
            window.printpdf.printSlider("#db-data")
          ],
          //Column 2
          [
            window.printpdf.printSlider("#retention")
          ]  
        ]
      //Name of Class in PDF output
      ], "#input-sliders");
      ...
      // *******************************
      data += "<h1>RESULTS:</h1>";
      data += "<div class='results discbackup'>";
      data += $(".results").html();
      data += "</div>";
